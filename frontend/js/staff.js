// CRUD de productos y tablero de pedidos
...
new Vue({
  ...
  data:()=>({
    ...
    filtro:{ cat:'', uni:'', q:'' }, enviandoProd:false, errorProd:'', msg:'',
  ...
  methods:{
    ...
    guardar(){
      this.errorProd=''; this.msg='';
      if(!this.form.nombre || !this.form.precio){ this.errorProd='Completá nombre y precio.'; return; }
      this.enviandoProd=true;
      const data = { nombre:this.form.nombre, precio:Number(this.form.precio)||0, categoria:this.form.categoria, uni:this.form.uni };
      const req = this.form.id ? axios.put(API+'/api/productos/'+this.form.id, data)
                               : axios.post(API+'/api/productos', data);
      req.then(()=>{ this.msg='Guardado'; this.cancelarEdicion(); this.cargarProductos(); })
    ...
    borrar(p){
      ...
      this.enviandoProd=true; this.msg='';
      axios.delete(API+'/api/productos/'+p.id)
    ...
    cargarPedidos(){
      ...
      const url = API+'/api/pedidos' + (q.length?('?'+q.join('&')):'');
      axios.get(url)
    ...
    setEstado(pedido, estado){
      ...
      axios.put(API+'/api/pedidos/'+pedido.id, {estado})
    ...
    setPago(pedido, estado){
      this.cargandoPed=true; this.errorPed='';
      axios.put(API+'/api/pedidos/'+pedido.id, {pago:estado})
        .then(()=> this.cargarPedidos())
